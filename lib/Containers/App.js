            ...
            <div className="bgg-collection">
                <div className={classnames(controlClasses)}>
                    ...
                    <form onSubmit={(ev)=>{
                        ...
                        <div className={classnames("bgg-collection__filters", {
                            "bgg-collection__filters--hidden": this.props.collection.length === 0
                            })}>
                            <Control fieldName="liveSearch" fieldType="text" onChange={_.throttle(this.handleFilter.bind(this), 50)}>
                                <label>Text search</label>
                            </Control>
                            <Control fieldName="year" fieldType="text" onChange={_.throttle(this.handleFilter.bind(this), 50)}>
                                <label>Year</label>
                            </Control>
...
function filterVisibleCollection(collection, filters = []) {
    ...
    let collections = filters.map((filterObj)=>{
        ...
        switch (filter) {
            ...
            case 'year':
                return collection.filter((item)=>{
                    return (value ? item.yearPublished === parseInt(value) : true);
                });
            case 'plays':
                return collection.filter((item)=>{
